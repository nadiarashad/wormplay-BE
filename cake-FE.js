...
function init() {
  ...
  var viewModelMaker = function () {
    ...
    // returns player to room list // lobby
  ...
  // connect to server
  socket = io.connect(
    location.protocol + "//" + location.host + "/sockets/cakerush"
  );
...
function handleImageLoad() {
  ...
  var spriteData = {
    images: [
      "/images/cakerush/cake.png",
      "/images/cakerush/fatBlue.png",
      "/images/cakerush/fatRed.png",
      "/images/cakerush/fatYellow.png",
      "/images/cakerush/fatGreen.png",
    ],
    frames: [
...
function startGame(data) {
  ...
  addNewPlayer({
    id: localPlayerId,
    spritex: Math.floor(
      Math.random() * (STAGE_RIGHT_X_BOUND - STAGE_LEFT_X_BOUND) +
        STAGE_LEFT_X_BOUND
    ),
    spritey: Math.floor(
      Math.random() * (STAGE_BOTTOM_Y_BOUND - STAGE_TOP_Y_BOUND) +
        STAGE_TOP_Y_BOUND
    ),
    updown: 0,
    leftright: 0,
...
function tick() {
  ...
  if (now - lastHealthPenaltyTime > 500) {
    ...
    if (iGotCake) {
      var localCakeIndex = _.indexOf(
        characters,
        _.find(characters, { id: "cake" })
      );
  ...
  for (var i = 0; i < characters.length; i++)
    if (characters[i]) characters[i].move(deltaTime);
...
function handleKeySignals(e, switchHandler) {
  ...
  if (
    !nonGameKeyPressed &&
    (lastLeftright != player.leftright ||
      lastUpdown != player.updown ||
      player.justAttacked)
  )
...
function sendGameDataToServer() {
  ...
  var player = _.find(characters, { id: localPlayerId });
  if (player) player.appendDataToMessage(data);
